...
router.post('/languages', async (req, res) => {
  const isBodyValid = checkBody(req.body, [
    'displayValue',
    'editorValue',
    'apiValue',
  ]); // isExecutable not included because if false would not pass checkBody
...
router.get('/languages', async (req, res) => {
  try {
    ...
    if (!devLangs || devLangs.length === 0)
      throw new Error('Could not retrieve dev languages');
...
router.get('/editor_themes', async (req, res) => {
  try {
    ...
    if (!editorThemes || editorThemes.length === 0)
      throw new Error('Could not retrieve themes');
